...
// Los descuentos guardados de Ajustes → Tarifas.
//
// Los servicios vivian aqui tambien, en un JSON con el precio dentro. Ya no:
// todo lo que vendes esta en `bonos_tipos` y su precio en `planes`.
//
...
export async function cargarTarifas() {
  const { data, error } = await supabase.from('ajustes').select('clave,valor')
    .in('clave', ['descuentos_lista'])
  if (error) {
    console.error('No se han podido leer los descuentos:', error.message)
    return { descuentos: DESCUENTOS_POR_DEFECTO, error: error.message }
  }
