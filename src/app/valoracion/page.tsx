        ...
        <div className="g2">
          ...
          <div>
            ...
            {(form.patologias.length>0||form.operaciones.length>0)&&<div style={{background:'var(--bl)',border:'1px solid var(--bd)',borderRadius:7,padding:'10px 12px',marginBottom:7}}>
              ...
              {form.patologias.map((p,i)=><div key={i} style={{fontSize:10,color:'var(--n)',marginBottom:2}}>🏥 {p.nombre} · <span style={{color:'var(--grl)'}}>{p.estado}</span></div>)}
              {form.operaciones.map((op,i)=><div key={i} style={{fontSize:10,color:'var(--n)',marginBottom:2}}>🔪 {op.nombre}{op.anio&&' · '+op.anio}</div>)}
            </div>}
            <div style={{background:'var(--gl)',border:'1px solid var(--gm)',borderRadius:7,padding:'10px 12px',marginBottom:12,fontSize:10,color:'var(--gd)'}}>
              ✓ Los datos quedarán guardados en la ficha del paciente.
            </div>
