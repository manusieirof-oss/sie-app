...
export default function PasoTests({ testsLib, etiquetasLib=[], testsValoracion, setTestsValoracion, testActivo, setTestActivo }: any) {
  // Elegir tests se hace en una pantalla aparte y no en una caja de 220px dentro del
  // paso. Con 49 tests, el hueco de antes enseñaba cuatro y obligaba a rascar con el
  // dedo; aquí caben veinte y se filtran por zona.
  ...
  /** La ficha vacía de un test recién añadido. Estaba escrita dos veces en este mismo fichero. */
  function nuevoTv(t:any){
    ...
    const ladoVacio = () => ({
      items_resultado:(t.items||[]).map((item:any)=>({...item,marcado:false,valor:''})),
      resultado:'sin_realizar', observaciones:'', fecha_repeticion:rev.toISOString().split('T')[0],
    })
    return { test_id:t.id, nombre:t.nombre, logica:t.logica, ladoActivo:ladoIni,
      frecuencia_meses:t.frecuencia_meses||3, lados:{ [ladoIni]: ladoVacio() } }
  }

  function confirmarSeleccion(){
    const nuevos = seleccion
      .map((id:string)=>testsLib.find((t:any)=>t.id===id))
      .filter(Boolean)
      .map(nuevoTv)
    if (nuevos.length) {
  ...
  return (
    <div>
      {testsValoracion.length===0 && (
        <div style={{background:'var(--bl)',border:'1.5px dashed var(--bm)',borderRadius:'var(--rl)',padding:'26px 20px',textAlign:'center',marginBottom:8}}>
          <div style={{color:'var(--grl)',marginBottom:8}}><Ic name="test" size={26}/></div>
          <div style={{fontSize:12,color:'var(--n)',marginBottom:4}}>Todavía no hay tests en esta valoración</div>
          <div style={{fontSize:10,color:'var(--grl)',marginBottom:12}}>Búscalos por zona o por el nombre de la maniobra.</div>
              ...
              <div style={{marginTop:12}}>
                ...
                {(testLib?.items||[]).length>0 ? (
                  <div style={{marginBottom:10}}>
                    {(d.items_resultado.length?d.items_resultado:(testLib.items||[]).map((item:any)=>({...item,marcado:false,valor:''}))).map((item:any,ii:number)=>{
                      const baseItems = d.items_resultado.length?d.items_resultado:(testLib.items||[]).map((it:any)=>({...it,marcado:false,valor:''}))
                      return (
                          ...
                          {mide(item)&&item.marcado&&(
                            <div style={{display:'flex',alignItems:'center',gap:4}}>
                              <input type="number" value={valorDe(item)} onChange={e=>{
                                const its=[...baseItems]; its[ii]={...its[ii],valor:e.target.value}
                              ...
                              <span style={{fontSize:10,color:'var(--red)'}}>{unidadDe(item).simbolo.trim()}</span>
                            </div>
                          )}
                        </div>
                      )
      ...
      {testsValoracion.length>0 && (
        <button className="btn btn-s" style={{width:'100%',justifyContent:'center',padding:'10px'}} onClick={()=>setAbierto(true)}>
