...
export default function AgendaPage() {
  ...
  async function cargarPacientes() {
    const { data } = await supabase2.from('pacientes').select('id,nombre,apellidos').eq('estado','activo').order('nombre')
    setPacientes(data||[])
  }
  ...
  async function cargar() {
    ...
    const { data: c } = await supabase2.from('citas').select('*, pacientes(id,nombre,apellidos,telefono,email,tipo_clase)').gte('fecha',fechaInicio).lte('fecha',fechaFin).neq('estado','cancelada').order('fecha').order('hora')
    setCitas(c||[])
    setLoading(false)
  }

  function getCitasSlot(h: string, sala: string, f?: string) {
    const fechaBuscar = f || fecha
    return citas.filter(c => c.hora.startsWith(h) && c.sala===sala && c.fecha===fechaBuscar)
  }
  ...
  function getDiasMes() {
    ...
    for(let i=1;i<=ultimo.getDate();i++) {
      dias.push(`${anio}-${String(mes).padStart(2,'0')}-${String(i).padStart(2,'0')}`)
    }
    return dias
  }
  ...
  async function crearCita() {
    ...
    if (!nuevaCita.repetir) {
      // Cita única
      await supabase2.from('citas').insert({
        paciente_id: nuevaCita.paciente_id,
        hora: nuevaCita.hora+':00', sala: nuevaCita.sala,
        tipo: nuevaCita.tipo, notas: nuevaCita.notas, fecha,
        duracion_min: nuevaCita.tipo==='valoracion'?60:50, estado:'programada',
      })
    } else {
      // Citas recurrentes
      ...
      while (fechaActual <= fechaFinObj) {
        ...
        if (diaStr && nuevaCita.dias_repetir.includes(diaStr)) {
          citasACrear.push({
            paciente_id: nuevaCita.paciente_id,
            hora: nuevaCita.hora+':00', sala: nuevaCita.sala,
            tipo: nuevaCita.tipo, notas: nuevaCita.notas,
      ...
      if (citasACrear.length>0) {
        ...
        for (let i=0;i<citasACrear.length;i+=50) {
          await supabase2.from('citas').insert(citasACrear.slice(i,i+50))
        }
        alert(`✓ ${citasACrear.length} citas creadas correctamente`)
      }
    }
  ...
  async function cambiarEstado(id: string, estado: string) {
    await supabase2.from('citas').update({ estado }).eq('id', id)
    cargar()
  }
  ...
  const toggleDia = (dia: string) => {
    setNuevaCita(p=>({...p, dias_repetir: p.dias_repetir.includes(dia) ? p.dias_repetir.filter(d=>d!==dia) : [...p.dias_repetir, dia]}))
  }
                        ...
                        <div key={sala} style={{borderLeft:'1px solid var(--bl)',padding:3,minHeight:52}}>
                          {slotCitas.length===0 ? (
                            ...
                            <div style={{borderRadius:4,padding:'3px 5px',background:tipo==='valoracion'||tipo==='revaloracion'?'var(--ambl)':'var(--gl)',borderLeft:`2px solid ${tipo==='valoracion'||tipo==='revaloracion'?'var(--amb)':'var(--g)'}`}}>
                              ...
                              {slotCitas.map(c=>(
                                <div key={c.id} onClick={(e)=>{e.stopPropagation();setPanelPac(c)}}
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {HORAS.map(h=>(
                    <div key={h}>
                      {h==='15:30' && <div style={{gridColumn:'1/-1',padding:'3px 10px',background:'var(--bm)',borderBottom:'1px solid var(--bd)',fontSize:8,color:'var(--gr)'}}>— Pausa 12:30–15:30</div>}
                              ...
                              {citasDia.length===0 ? (
                                ...
                                <div style={{borderRadius:3,padding:'2px 3px',background:'var(--gl)',borderLeft:'2px solid var(--g)'}}>
                                  {citasDia.slice(0,3).map(c=>(
                                    <div key={c.id} onClick={()=>setPanelPac(c)} style={{fontSize:8,color:'var(--n)',fontWeight:300,padding:'1px 2px',cursor:'pointer',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                                      {c.pacientes?.nombre}
                                    </div>
                                  ))}
        ...
        <>
          ...
          <div style={{position:'fixed',top:0,right:0,width:300,height:'100vh',background:'var(--w)',borderLeft:'1px solid var(--bd)',zIndex:99,display:'flex',flexDirection:'column',boxShadow:'-4px 0 20px rgba(38,40,37,.08)'}}>
            ...
            <div style={{flex:1,overflowY:'auto',padding:14}}>
              ...
              <div style={{marginTop:10,marginBottom:8,fontSize:9,fontWeight:600,color:'var(--grl)',letterSpacing:.5,textTransform:'uppercase'}}>Estado de la cita</div>
              ...
              {panelPac.notas && <div style={{marginTop:12,padding:'7px 9px',background:'var(--ambl)',borderRadius:5,border:'1px solid var(--amb)',fontSize:10,color:'#7A5800'}}>{panelPac.notas}</div>}
            </div>
          ...
          <div className="modal" style={{width:460}}>
            ...
            <div style={{background:'var(--bl)',border:'1px solid var(--bd)',borderRadius:'var(--rl)',padding:'10px 12px',marginBottom:10}}>
              ...
              {nuevaCita.repetir && (
                <>
                    ...
                    <div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:4}}>
                      {DIAS_SEMANA.map(d=>(
                        <button key={d} onClick={()=>toggleDia(d)}
                          style={{fontSize:10,padding:'4px 9px',borderRadius:99,border:`1px solid ${nuevaCita.dias_repetir.includes(d)?'var(--g)':'var(--bd)'}`,background:nuevaCita.dias_repetir.includes(d)?'var(--g)':'var(--w)',color:nuevaCita.dias_repetir.includes(d)?'#fff':'var(--gr)',cursor:'pointer',fontFamily:'system-ui',transition:'all .12s'}}>
                          {d}
                        </button>
                      ))}
                    </div>
                  ...
                  <div style={{background:'var(--gl)',borderRadius:5,padding:'6px 9px',fontSize:9,color:'var(--gd)'}}>
                    ✓ Se crearán automáticamente todas las citas para los días seleccionados
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{display:'flex',gap:8,marginTop:8}}>
              ...
              <button className="btn btn-p" onClick={crearCita} disabled={guardando} style={{opacity:guardando?0.7:1}}>
                {guardando?'⏳ Creando citas...':'✓ Crear cita'}
              </button>
            </div>
          </div>
        </div>
