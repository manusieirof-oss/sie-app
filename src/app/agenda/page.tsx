                ...
                <>
                  <div style={{display:'grid',gridTemplateColumns:'48px repeat(6,1fr)',background:'var(--bl)',borderBottom:'1px solid var(--bd)',position:'sticky',top:0,zIndex:2}}>
                    ...
                    {fs.map((f,i)=>{const isH=f===hoy,d=new Date(f+'T12:00:00');return(
                      <div key={f} onClick={()=>{setFecha(f);setVista('dia')}} style={{padding:'6px 4px',textAlign:'center',borderLeft:'1px solid var(--bd)',cursor:'pointer',background:isH?'var(--gl)':'transparent'}}>
                        <div style={{fontSize:8,color:'var(--grl)',fontWeight:300}}>{dn[i]}</div>
                        <div style={{fontSize:13,fontWeight:isH?600:300,color:isH?'var(--g)':'var(--n)'}}>{d.getDate()}</div>
                      </div>
                    )})}
                  </div>
                  {HORAS.map(h=>(
                    <div key={h}>
                      ...
                      <div style={{display:'grid',gridTemplateColumns:'48px repeat(6,1fr)',borderBottom:'1px solid var(--bl)',minHeight:44}}>
                        ...
                        {fs.map(f=>{
                          const cd=citas.filter(c=>c.fecha===f&&c.hora.startsWith(h)),isH=f===hoy
                          return (
                            <div key={f} style={{borderLeft:'1px solid var(--bl)',padding:2,background:isH?'rgba(90,150,158,.03)':'transparent',minHeight:44}}>
                              {cd.length===0?(
                                ...
                                <div style={{borderRadius:3,padding:'2px 3px',background:'var(--gl)',borderLeft:'2px solid var(--g)'}}>
                                  {cd.slice(0,3).map(c=>(
                                    <div key={c.id} onClick={()=>abrirPanel(c)} style={{fontSize:8,color:'var(--n)',fontWeight:300,padding:'1px 2px',cursor:'pointer',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{c.pacientes?.nombre}</div>
                                  ))}
                                  {cd.length>3&&<div style={{fontSize:8,color:'var(--g)'}}>+{cd.length-3} más</div>}
                                </div>
                              )}
                            </div>
                          )
