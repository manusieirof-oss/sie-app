    ...
    <div className="g2">
      <div>
            ...
            <div key={m.id} style={{borderRadius:8,padding:'8px 10px',marginBottom:5,border:'1px solid',borderColor:m.activa?'#F5C8C8':'var(--gm)',backgroundColor:m.activa?'var(--redl)':'var(--gl)'}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{flex:1,cursor:'pointer'}} onClick={()=>setDetalle({tipo:'molestia',datos:m})}>
                  ...
                  <div style={{fontSize:10,color:'var(--grl)'}}>EVA {m.eva}/10 · {m.tipo?.replace('_',' ')} · <span style={{color:'var(--g)'}}>ver detalle</span></div>
                </div>
                <span style={{fontSize:9,fontWeight:500,padding:'2px 7px',borderRadius:99,background:m.activa?'var(--redl)':'var(--gl)',color:m.activa?'var(--red)':'var(--gd)'}}>
          ...
          {patologias.map((p:any)=>(
            <div key={p.id} className="ri">
              ...
              <div style={{flex:1,cursor:'pointer'}} onClick={()=>setDetalle({tipo:'patologia',datos:p})}>
                ...
                <div style={{fontSize:10,color:'var(--grl)'}}>{(p.lado&&p.lado!=='no_aplica')?p.lado+' · ':''}<span style={{color:'var(--g)'}}>ver detalle</span></div>
              </div>
        ...
        <div className="card">
          ...
          <div onClick={()=>setUsaPlantillas(!usaPlantillas)} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',borderRadius:8,border:`1px solid ${usaPlantillas?'var(--g)':'var(--bd)'}`,background:usaPlantillas?'var(--gl)':'var(--w)',cursor:'pointer',marginBottom:8}}>
          ...
          <button className="btn btn-p btn-sm" onClick={guardarSalud} disabled={guardandoSalud} style={{marginTop:4}}>{guardandoSalud?'…':<><Ic name="guardar" size={12}/> Guardar plantillas</>}</button>
        </div>
      ...
      <div>
        <div className="card">
          ...
          {escalas.length===0&&<div className="muted">Sin escalas registradas</div>}
          {escalas.map((e:any)=>(
            <div key={e.id} style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
              <span style={{fontSize:10,color:'var(--grl)',width:50,fontWeight:300}}>{new Date(e.fecha+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short'})}</span>
              <div style={{flex:1}}>
                <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
                  <span style={{fontSize:10,color:'var(--grl)',width:44}}>Borg</span>
                ...
                <div style={{display:'flex',alignItems:'center',gap:5}}>
                  <span style={{fontSize:10,color:'var(--grl)',width:44}}>Estrés</span>
        ...
        <div className="card">
          ...
          {medicamentos.map((m:any)=>(
            <div key={m.id} className="ri" style={{cursor:'pointer'}} onClick={()=>setDetalle({tipo:'medicamento',datos:m})}>
              ...
              <div><div style={{fontSize:11,fontWeight:400,color:'var(--n)'}}>{m.nombre}</div><div style={{fontSize:10,color:'var(--grl)'}}>{m.frecuencia||'ver detalle'}</div></div>
            </div>
          ))}
        </div>
            ...
            return (
              <div className="g2">
                <div>
                  ...
                  {positivos.length===0&&<div style={{fontSize:10,color:'var(--grl)',marginBottom:8}}>Sin tests positivos</div>}
                  {positivos.map((grupo:any,gi:number)=>{
                    const t=grupo[0]; const anteriores=grupo.slice(1)
                    return (
                      <div key={gi} style={{padding:'7px 10px',background:'var(--redl)',borderRadius:8,border:'1px solid #F5C8C8',marginBottom:6}}>
                        <div style={{display:'flex',alignItems:'center',gap:7}}>
                          <div style={{flex:1}}>
                            ...
                            <div style={{fontSize:10,color:'var(--grl)',marginTop:1}}>{new Date(t.fecha+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short',year:'numeric'})} · {grupo.length} {grupo.length===1?'registro':'registros'}</div>
                          </div>
                          <span style={{fontSize:9,fontWeight:500,padding:'2px 7px',borderRadius:99,background:'var(--redl)',color:'var(--red)',border:'1px solid var(--red)'}}>+ Positivo</span>
                          <button onClick={()=>resolverTestNegativo(t)} style={{fontSize:9,padding:'2px 6px',borderRadius:3,border:'1px solid var(--g)',background:'var(--gl)',color:'var(--gd)',cursor:'pointer',fontFamily:'system-ui'}}>→ Negativo</button>
                        </div>
                        {(t.items_resultado||[]).filter((i:any)=>i.marcado).map((item:any,ii:number)=>(
                          <div key={ii} style={{fontSize:9,color:'var(--red)',marginTop:3,display:'flex',alignItems:'center',gap:5}}><Ic name="checkbox" size={11}/><span>{item.nombre}{item.grados?' · '+item.grados+'°':''}</span></div>
                        ))}
                        {t.observaciones&&<div style={{fontSize:9,color:'var(--gr)',marginTop:4,fontStyle:'italic'}}>{t.observaciones}</div>}
                        {t.fecha_repeticion&&<div style={{fontSize:9,color:'var(--amb)',marginTop:2,display:'flex',alignItems:'center',gap:4}}><Ic name="alarma" size={11}/> Revisión: {new Date(t.fecha_repeticion+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short'})}</div>}
                        {anteriores.length>0&&<details style={{marginTop:6}}><summary style={{fontSize:10,color:'var(--grl)',cursor:'pointer',listStyle:'none'}}>▸ Historial ({anteriores.length} anterior{anteriores.length>1?'es':''})</summary><div style={{marginTop:5,paddingLeft:8,borderLeft:'2px solid #F5C8C8'}}>{anteriores.map((ant:any,ai:number)=><div key={ai} style={{marginBottom:5,padding:'5px 8px',background:'rgba(255,255,255,.6)',borderRadius:4}}><div style={{fontSize:9,fontWeight:400,color:'var(--n)',marginBottom:2}}>{new Date(ant.fecha+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short',year:'numeric'})} · <span style={{color:ant.resultado==='positivo'?'var(--red)':'var(--g)'}}>{ant.resultado==='positivo'?'+ Positivo':'− Negativo'}</span></div>{ant.observaciones&&<div style={{fontSize:9,color:'var(--grl)',fontStyle:'italic'}}>{ant.observaciones}</div>}</div>)}</div></details>}
                      </div>
                    )
                  })}
                </div>
                <div>
                  ...
                  {negativos.length===0&&<div style={{fontSize:10,color:'var(--grl)',marginBottom:8}}>Sin tests negativos</div>}
                  {negativos.map((grupo:any,gi:number)=>{
                    const t=grupo[0]; const anteriores=grupo.slice(1)
                    return (
                      <div key={gi} style={{padding:'7px 10px',background:'var(--gl)',borderRadius:8,border:'1px solid var(--gm)',marginBottom:6}}>
                        <div style={{display:'flex',alignItems:'center',gap:7}}>
                          <div style={{flex:1}}>
                            <div style={{fontSize:11,fontWeight:400,color:'var(--n)'}}>{t.tests?.nombre||'Test'}{t.lado&&t.lado!=='bilateral'?' · '+t.lado.charAt(0).toUpperCase()+t.lado.slice(1):''}</div>
                            <div style={{fontSize:10,color:'var(--grl)',marginTop:1}}>{new Date(t.fecha+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short',year:'numeric'})} · {grupo.length} {grupo.length===1?'registro':'registros'}</div>
                          </div>
                          <span style={{fontSize:9,fontWeight:500,padding:'2px 7px',borderRadius:99,background:'var(--gl)',color:'var(--gd)',border:'1px solid var(--gm)'}}>− Negativo</span>
                        </div>
                        {t.observaciones&&<div style={{fontSize:9,color:'var(--gr)',marginTop:3,fontStyle:'italic'}}>{t.observaciones}</div>}
                        {anteriores.length>0&&<details style={{marginTop:6}}><summary style={{fontSize:10,color:'var(--grl)',cursor:'pointer',listStyle:'none'}}>▸ Historial ({anteriores.length} anterior{anteriores.length>1?'es':''})</summary><div style={{marginTop:5,paddingLeft:8,borderLeft:'2px solid var(--gm)'}}>{anteriores.map((ant:any,ai:number)=><div key={ai} style={{marginBottom:5,padding:'5px 8px',background:'rgba(255,255,255,.6)',borderRadius:4}}><div style={{fontSize:9,fontWeight:400,color:'var(--n)',marginBottom:2}}>{new Date(ant.fecha+'T12:00:00').toLocaleDateString('es-ES',{day:'numeric',month:'short',year:'numeric'})} · <span style={{color:ant.resultado==='positivo'?'var(--red)':'var(--g)'}}>{ant.resultado==='positivo'?'+ Positivo':'− Negativo'}</span></div>{ant.observaciones&&<div style={{fontSize:9,color:'var(--grl)',fontStyle:'italic'}}>{ant.observaciones}</div>}</div>)}</div></details>}
                      </div>
                    )
                  })}
                </div>
      ...
      {/* MODAL DETALLE */}
      {detalle&&<div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)setDetalle(null)}}><div className="modal"><div className="modal-title"><span className="ct-l"><Ic name={detalle.tipo==='molestia'?'molestia':detalle.tipo==='patologia'?'patologia':'medicamento'} size={16}/> {detalle.tipo==='molestia'?(detalle.datos.zona||'Molestia'):detalle.tipo==='patologia'?(detalle.datos.nombre||'Patología'):(detalle.datos.nombre||'Medicamento')}</span><button className="modal-close" onClick={()=>setDetalle(null)}>✕</button></div>
